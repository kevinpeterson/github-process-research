...
In addition, three supporting research questions are proposed. These questions will draw upon data from this work, but also reference several case studies to better represent how GitHub may be changing how OSS development is conducted. These qualitative observations, paired with the more qualitative data analysis, allows for a broad description of the way that GitHub is changing the Software Engineering landscape for OSS.
...
A \emph{fork} is the creation of a personal copy of a repository\cite{dabbish2012social}, from which they can change without impacting the original code source. A \emph{fork} is an \emph{active} action, as it signifies both involvement and intent to contribute. It should be noted, however, that it only signifies \emph{intent} -- a fork of a repository could be made and no other actions taken.
...
Repositories were gathered at random, but only repositories owned by an \textit{Organization} GitHub User type were considered. The \textsc{isOrgRepo} function will filter out all non \textit{Organization} owned repositories.
...
All analyzed \textit{Respositories} can be thought of as the Indexed Family of \textit{Attribute} Sets $A$, or a set of sets of \textit{Attributes}. Let the set of these sets of \textit{Attributes} be called $S$, or the \textit{Repository Sample}. We can label the \textit{Attribute} sets by arbitrary integers 0 through $|S|$, or:
\[ \{A_i\}_{i \in \{0,1,2 ... |S|\} }\]
...
Each \textit{Committers}, then, will produce a subset of all \textit{Commits} to a a \textit{Repository}, or $C_u \subset \mathscr{C}$. 
...
Despite the potential migration cost, the National Center for Biomedical Ontology,\footnote{https://github.com/ncbo}, the National Cancer Informatics Program\footnote{https://github.com/ncip}, NASA\footnote{https://github.com/nasa}, and the WhiteHouse\footnote{https://github.com/WhiteHouse} are just some organizations that have moved all or portions of development to GitHub. As these and other organizations consider the move to GitHub, the distributed and ``social'' aspects of the OSS process will move continue to evolve.\\
...
The data summary variance statement in Assertion: \ref{eq:variance} is important, as it suggests large amounts of variety in the observed variables. This may suggest that not all repositories on GitHub are intended to be consumable software artifacts for users, or ``consumers.'' A large number of repositories may not be intended for customer consumption, but merely as example code, experimentation, or exercises. More research into categorizing GitHub repositories is needed. This is one reason why only repositories owned by a GitHub \textit{Organization} were analyzed, as it was anticipated that this would yield a better sample of repositories for analysis.

The non-code artifacts in GitHub may fall outside of the domain of software development, but it is a significant aspect of our findings. From government documents to software specifications, GitHub is being used and used successfully. More research is needed into what role GitHub will play in the work flow of these artifacts, and whether it will influence their traditional work flow, as it has for software development.

...
To gather the metics, each individual Attribute processed by some aggregate function $f$, or $f(  A_{i,r} )$. For this analysis, the aggregate functions are: \textsc{mean}, \textsc{min}, \textsc{max}, \textsc{stddev}. This analysis is applied to all $n$ repositories for which data was collected. Results were then averaged over all repositories. Each attribute is analyzed independently, so given:
...
\[ \Delta = d\left( \frac{\sum_{i \in I} s( p_{(2)}i ) - s( p_{(1)}i )  } {n} \right) \]
...
We thank Gilberto Fragoso, Ph.D., Paul Alexander, and the GitHub Staff for insight and ideas, Harold Solbrig, Craig Stancl, and Dick Hedger for guidance, and Rick Kiefer for his review.
